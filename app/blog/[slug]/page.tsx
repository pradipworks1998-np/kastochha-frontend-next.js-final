  ...
  const renderOptions: Options = {
    renderNode: {
      [INLINES.HYPERLINK]: (node, children) => (
        <a href={node.data.uri} target="_blank" className="text-blue-600 underline font-medium hover:text-blue-800">{children}</a>
      ),
      ...
      [BLOCKS.EMBEDDED_ASSET]: (node) => (
        <img src={`https:${node.data.target.fields.file.url}`} alt={node.data.target.fields.title} className="rounded-2xl my-10 w-full shadow-sm" />
      ),
    },
  };
    ...
    <div className="flex flex-col min-h-screen pt-6 md:pt-10 lg:pt-12 bg-white">
      ...
      <main className="flex-1 max-w-6xl mx-auto px-6 pt-2 pb-24 w-full">
        <header className="max-w-4xl mx-auto text-left mb-12">
          <nav className="mb-4 flex flex-wrap items-center justify-between gap-4">
             ...
             <div className="flex gap-2">
               {(fields as any).categories?.map((cat: string) => (
                 <span key={cat} className="px-3 py-1 bg-slate-100 text-slate-600 text-[9px] font-bold uppercase tracking-widest rounded-full">
                   {cat}
                 </span>
               ))}
             </div>
          ...
          <h1 className="text-3xl md:text-5xl font-extrabold text-slate-900 mb-6 italic leading-tight">
            {fields.title}
          </h1>
          ...
          <div className="flex items-center justify-start gap-3 text-xs md:text-sm text-slate-500 mb-8 font-medium">
            ...
            <span className="opacity-30">|</span>
            <span>
                {new Date((fields as any).publishedAt).toLocaleDateString('en-US', { 
                    month: 'short', 
                    day: 'numeric', 
                    year: 'numeric' 
                })}
        ...
        {/* --- DYNAMIC LAYOUT: SPLIT TOC FOR MOBILE TOP / DESKTOP SIDEBAR --- */}
        <div className="flex flex-col lg:flex-row gap-12 items-start">
          
          {/* A. Mobile Table of Contents (order-1, hidden on Desktop) */}
          {headings.length > 0 && (
          ...
          {/* B. Main Article Content (order-2 on mobile) */}
          ...
          {/* C. Sidebar (order-3 on mobile, lg:order-2 on desktop) */}
            ...
            <div className="p-6 kasto-gradient rounded-3xl text-white shadow-xl">
              <div className="flex items-center justify-between mb-6">
                ...
                <div className="flex gap-4">
                    <a href={`https://www.facebook.com/sharer/sharer.php?u=${shareUrl}`} target="_blank" className="hover:scale-110 transition-transform">
                    ...
                    <a href={`https://twitter.com/intent/tweet?url=${shareUrl}`} target="_blank" className="hover:scale-110 transition-transform">
                        <svg className="w-5 h-5 fill-current" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
                    </a>
                    <a href={`https://www.linkedin.com/sharing/share-offsite/?url=${shareUrl}`} target="_blank" className="hover:scale-110 transition-transform">
